"""
Attack: Seme+DQPSO — BERT on IMDB  (multi-worker version)
============================================================
Search space  : HowNet sememe-based substitutes
Search method : Discrete QPSO  — Equation (15) from the paper

This version processes multiple IMDB samples CONCURRENTLY using separate
worker processes (num_workers, similar in spirit to TextAttack's
num_workers_per_device), each sharing the same GPU. This keeps the GPU
busier than running samples strictly one-after-another, since while one
worker is doing Python-side bookkeeping between batched calls, other
workers can be issuing their own GPU calls.
...
Usage:
    python attack_dqpso_bert_imdb.py                              # default: 4 workers
    python attack_dqpso_bert_imdb.py --num_workers 2   # fewer workers if VRAM is tight
"""
...
RESULTS_FILE = "results_dqpso_bert_imdb.csv"
...
# ======================================================================= #
#  DQPSO POSITION UPDATE  (Equation 15)                                   #
# ======================================================================= #

def dqpso_update(swarm, personal_best, global_best, S0, search_space, alpha_t):
    M = len(swarm)
    D = len(S0)
    ...
    for i in range(M):
        ...
        p_it = [phi[j] * I_pbest[i][j] + (1 - phi[j]) * I_gbest[j]
                for j in range(D)]

        sign  = [1 if random.random() > 0.5 else -1 for _ in range(D)]
        u     = [max(1e-9, random.random()) for _ in range(D)]
        X_new = [
            p_it[j] + sign[j] * alpha_t * abs(C_t[j] - I_xi[j]) * math.log(1.0 / u[j])
            for j in range(D)
        ]
...
# ======================================================================= #
#  FULL DQPSO ATTACK FOR ONE SENTENCE                                     #
# ======================================================================= #

def attack_sentence(S0_tokens, true_label, victim, hownet_dict, T=20, M=60):
    ...
    for t in range(1, T + 1):
        ...
        swarm = dqpso_update(swarm, personal_best, global_best,
                             S0_tokens, search_space, alpha_t)
...
def append_row_safe(path, lock_path, row: dict):
    """Process-safe append — multiple workers can call this concurrently."""
    with FileLock(lock_path):
        with open(path, "a", newline="", encoding="utf-8") as f:
            csv.DictWriter(f, fieldnames=CSV_COLUMNS).writerow(row)

def load_done_indices(path):
    """Return the set of sample_idx values already present in the CSV."""
    if not os.path.exists(path):
        return set()
    done = set()
...
def worker_process(worker_id, sample_indices, samples, model_path,
    ...
    """
    Each worker loads its OWN copy of the model and HowNet dict, then
    processes its assigned slice of sample indices independently. Several
    workers share the same physical GPU (device_str), which keeps the GPU
    busier than fully sequential processing because while one worker does
    CPU-side bookkeeping between calls, others can issue GPU work.
    """
    ...
    for count, idx in enumerate(sample_indices):
        ...
        if orig_pred != label:
            ...
            adv_tokens = attack_sentence(tokens, label, victim, hownet_dict,
                                         T=20, M=60)
...
if __name__ == "__main__":
    ...
    if not remaining_indices:
        ...
        # Split remaining indices round-robin across workers so each worker
        # gets a mix of short/long samples rather than one slow contiguous chunk
    ...
    print("\n" + "=" * 54)
    print("   Seme+DQPSO — BERT / IMDB   METRICS REPORT")
    print("=" * 54)
